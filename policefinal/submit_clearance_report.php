...
$age = $_POST['age'];
$nicNo = $_POST['nicNo'];
$confirmNicNo = $_POST['confirmNicNo'];
$newNicNo = $_POST['newNicNo'];
$confirmNewNicNo = $_POST['confirmNewNicNo'];
$passportNo = $_POST['passportNo'];
$confirmPassportNo = $_POST['confirmPassportNo'];
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO clearance_reports (application_type, name, nationality, citizen_of_sri_lanka, dob, left_sri_lanka, age, nic_no, confirm_nic_no, new_nic_no, confirm_new_nic_no, passport_no, confirm_passport_no, email, country, high_commission, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception("Prepare statement failed: " . $conn->error);
    }

    $stmt->bind_param("sssssisississssss", $applicationType, $name, $nationality, $citizenOfSriLanka, $dob, $leftSriLanka, $age, $nicNo, $confirmNicNo, $newNicNo, $confirmNewNicNo, $passportNo, $confirmPassportNo, $email, $country, $highCommission, $address);
    ...
    $mail->Password = 'jshv tdlb epqh tkgm'; // SMTP password
